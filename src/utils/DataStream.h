...
class EXPORT_MACRO DataStream {
 public:
    ...
    /// @brief  Retrieve the underlying bytebuffer as a char*
    ///
    /// Please note that this method will allocate a new char* of
    /// the size of the underlying buffer, if you want to handle
    /// allocation and deallocation yourself, please use
    /// void copToByteBuffer(char* outBuffer)
    /// @return a pointer to the byte buffer
    char* getByteBuffer();

    ///
    /// @brief Get the Byte Buffer object
    ///
    /// @param byteBuffer the return parameter where we will copy
    ///                   the underlying byte buffer. Please create it using
    ///                   DataStream::size() method
    void copToByteBuffer(char* byteBuffer);
    ...
    DataStream& operator<<(std::string c);
