...
DataStream::DataStream(Endian stream) : _buffer(), _read_pos(0), _write_pos(0), _stream_endian(stream), _machine_endian(LITTLE) {
    long one(1);
...
DataStream::DataStream(const char* buffer, size_t length, Endian stream)
    : _buffer(), _read_pos(0), _write_pos(0), _stream_endian(stream), _machine_endian(LITTLE) {
    long one(1);
    char e = (reinterpret_cast<char*>(&one))[0];
...
char* DataStream::getByteBuffer() {
    char* byteBuffer = new char[size()];
    copToByteBuffer(byteBuffer);
    return byteBuffer;
}

void DataStream::copToByteBuffer(char* byteBuffer) {
    // Expensive operation, copy _buffer on the out parameter
    for (int i = 0; i < size(); i++) {
        byteBuffer[i] = _buffer[i];
    }
}
...
DataStream& DataStream::operator<<(std::string d) {
    int stringSize = static_cast<int>(d.size());
    WriteAlgorithm(stringSize);
    for (int i = 0; i < stringSize; i++) {
        WriteAlgorithm(d.at(i));
    }
    return *this;
}

// read stuff
DataStream& DataStream::operator>>(std::string& c) {
    int size = 0;
    *this >> size;
    for (int i = 0; i < size; i++) {
        char character;
        ReadAlgorithm(character);
